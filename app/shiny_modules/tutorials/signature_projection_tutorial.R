  ...
  card(
    div(
      ...
      tags$ol(
        tags$li(
          ...
          tags$ul(
            ...
            tags$li(tags$strong("Sample to project"), ": the Salmon quantification file of the new sample to position; it is imported with tximport and merged with the reference before VST."),
        ...
        tags$li(
          ...
          tags$ul(
            tags$li("Optional: keep only the reference samples whose chosen gene sits below / above a given expression quantile (quantile 0 disables it).")
          )
        ),
        tags$li(
          tags$strong("Select signature *"),
          tags$ul(
            tags$li(tags$strong("Therapy / Signature"), ": the base signature, chosen by therapy then name."),
            tags$li(tags$strong("Combine with other signatures / genes"), ": optionally add components combined with the base score using ", tags$code("+"), ", ", tags$code("-"), ", ", tags$code("*"), " or ", tags$code("/"), ". Each component is a signature or a single gene. Operators are applied left to right, in the order shown (no math precedence).")
          )
        ),
        tags$li(
          tags$strong("Parameters *"),
          tags$ul(
            tags$li(tags$strong("Contrast"), ": a single clinical column holding the outcome."),
            tags$li(tags$strong("Responders / Non-Responders"), ": which modalities of the contrast column define each class. At least 2 samples per class are required after intersecting with the reference bulk.")
          )
        ),
        tags$li(
          ...
          tags$ul(
            ...
            tags$li(tags$strong("Event / Survival time columns"), ": required only when the KM plot is enabled."),
      ...
      tags$ul(
        ...
        tags$li(tags$strong("Projection"), ": the reference score distribution, the decision threshold and gray zone, and the projected sample with its calibrated probability of response."),
        tags$li(tags$strong("Survival analysis"), ": the Kaplan-Meier plot (only when KM is enabled)."),
        tags$li(tags$strong("Signature assessment"), ": the responder vs non-responder boxplot (Wilcoxon) and the ROC curve with AUC.")
      ),
      ...
      tags$ul(
        ...
        tags$li("If a class has fewer than 2 samples after filtering, the run stops with a detailed diagnostic explaining which step emptied it."),
